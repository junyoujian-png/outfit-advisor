...
class _FortuneScreenState extends State<FortuneScreen> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      final prompt =
          '你是一位專業星座運勢占卜師。請為「$_selectedLabel」提供今日運勢，'
      ...
      child: Column(
        ...
        children: [
          ...
          if (_fortune != null && !_loading) ...[
            const SizedBox(height: 24),
            _FortuneResult(fortune: _fortune!, label: _selectedLabel, emoji: _selectedEmoji),
          ],
        ],
      ),
    );
...
class _FortuneResult extends StatelessWidget {
  const _FortuneResult({required this.fortune, required this.label, required this.emoji});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        // 幸運色 + 幸運數字
        Row(children: [
          Expanded(child: _InfoCard(label: '🎨 幸運色', value: fortune['luckyColor'] ?? '')),
          const SizedBox(width: 10),
          Expanded(child: _InfoCard(label: '🔢 幸運數字', value: fortune['luckyNumber'] ?? '')),
        ]),
        const SizedBox(height: 10),

        // 愛情/事業/健康
        Row(children: [
          Expanded(child: _DetailCard(label: '💕 愛情運', text: fortune['love'] ?? '')),
          const SizedBox(width: 8),
          Expanded(child: _DetailCard(label: '💼 事業運', text: fortune['career'] ?? '')),
          const SizedBox(width: 8),
          Expanded(child: _DetailCard(label: '🌿 健康運', text: fortune['health'] ?? '')),
        ]),
      ],
    );
  }
...
MarkdownStyleSheet _mdStyle({double fontSize = 14}) => MarkdownStyleSheet(
      p: TextStyle(fontSize: fontSize, color: Colors.white.withValues(alpha: 0.88), height: 1.6),
      strong: const TextStyle(color: Color(0xFFE2D9FF), fontWeight: FontWeight.w700),
      h3: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white),
    );
